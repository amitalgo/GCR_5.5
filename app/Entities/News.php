...
class News
{
    ...
    /**
     * @ORM\OneToMany(targetEntity="NewsAttachment", mappedBy="newsId",cascade={"persist"})
     */
    private $NewsAttachment;

    /**
     * @ORM\OneToMany(targetEntity="NewsPages",fetch="EAGER", mappedBy="news" ,cascade={"persist"})
     */
    private $newsId;
    ...
    public  function addNewsPages(NewsPages $pageNews){
        if(!$this->newsId->contains($pageNews)){
            $pageNews->setNews($this);
            $this->newsId->add($pageNews);
        }
    }

    public function getSelectedPages(){
        $newsArray =[];
        foreach ($this->newsId as $news){
            $newsArray[]=$news->getPage()->getId();
