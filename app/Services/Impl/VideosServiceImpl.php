...
class VideosServiceImpl implements VideosService
{
    ...
    private $cat;
    private $tags;
    private $solutionTypeRepo,$uploadService;
    public function __construct(UploadService $uploadService,VideosRepo $videosRepo,CategoryRepo $cat,TagService $tagService,SolutionTypeRepo $solutionTypeRepo)
    {
...
//        }
    ...
    public function setArray($data)
    {
       ...
       for($i=0; $i<$arr_length;$i++){
           ...
           if(isset($input_array['r-link'][$i]) && $input_array['r-link'][$i]!=null){

               $exp_centre->setLinkRedirect($input_array['r-link'][$i]);

               $mulImage = $this->uploadService->UploadMulFile($data,'image','ExperienceCentre');
               if($mulImage[$i]==''){
                   $exp_centre->setMediaUrl($input_array['url'][$i]);
               }else{
                   $exp_centre->setMediaUrl($mulImage[$i]);
               }
           ...
           (isset($input_array['is_video'][$i]))?$exp_centre->setIsVideo($input_array['is_video'][$i]):$exp_centre->setIsVideo(1);
