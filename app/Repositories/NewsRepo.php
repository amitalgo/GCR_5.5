...
class NewsRepo
{
    ...
    public function addNews($data)
    {
        ...
        $news->setIsActive(1);
        // add/update news as per pages

        foreach ($data->get('pages') as $page){
            $pageNews = new NewsPages();
            $pageNews->setPage($this->em->getRepository(Page::class)->find($page));
            $pageNews->setIsActive(1);
            $pageNews->setDeleted(0);
            $pageNews->setCreatedAt(new \DateTime(now()));
            $news->addNewsPages($pageNews);
        }
    ...
    public function updateNews($data,$id)
    {
        ...
        // update news as per pages
        $deleteNewsRelatedPage=$this->em->getRepository(NewsPages::class)->findBy(["news"=>$news->getId()]);

        foreach ($deleteNewsRelatedPage as $pageDate) {
            $this->em->remove($pageDate);
        }
            $this->em->flush();
